...
#     pending: bool = False
#     note: Optional[str] = None
#     children: list["MetricRow"] = []   # NEW — nested child rows
...
#     pending: bool = False
#     note: Optional[str] = None
#     # Child rows shown when this row is expanded in the UI. [] = leaf row.
#     children: list["MetricRow"] = []


# MetricRow.model_rebuild()  # resolve the self-referential children type
...
class ShiftValues(BaseModel):
    """One metric's value across the three shifts and two day-totals.

    total          — operating-day total (06:00 -> 06:00 next day; sum of the
                     three shifts). Unchanged existing behaviour.
    calendar_total — strict calendar-day total (00:00 -> 24:00 IST of the
                     selected date). One number, no shift split. May differ from
                     `total` because the two 06:00 edge-slices contain different
                     data.
    """
    morning: Optional[float] = None
    afternoon: Optional[float] = None
    evening: Optional[float] = None
    total: Optional[float] = None
    calendar_total: Optional[float] = None
