...
def convert_ist_to_utc(value) -> str:
    ...
    if '/' in date_str:
        try:
            # First attempt native pandas datetime parsing with dayfirst prioritization
            dt = pd.to_datetime(date_str, dayfirst=True, errors='raise')
        except Exception:
            dt = pd.to_datetime(date_str, errors='coerce')
    else:
        try:
            # 1. Check for standard format like "25-Jun-26" or "25-Jun-2026"
            if '-' in date_str and len(date_str) >= 8:
                try:
                    dt = pd.to_datetime(date_str, format='%d-%b-%y', errors='raise')
                except Exception:
                    dt = pd.to_datetime(date_str, format='%d-%b-%Y', errors='raise')
            ...
            # Try a generic fallback before completely giving up
            try:
    ...
    try:
        ...
        # If it's just a Date without Time, tz_localize might whine. 
        # Convert to string format that Postgres expects: YYYY-MM-DD
...
if __name__ == "__main__":
    input_filename = 'Export_XRAY.csv'
    ...
    if header_idx is not None:
        ...
        # print(f" Date Column Indices Found: {date_col_indices}")
            ...
            # Temporary DataFrame assigning the detected row as columns for a pretty terminal view
            preview_df = df.iloc[header_idx + 1 : header_idx + 11].copy() # Grabs the first 10 rows
            preview_df.columns = original_row_headers
            
            # Filter columns to only show AWB and key Date Columns so it fits nicely in your terminal screen
            cols_to_show = []
            if awb_col_idx is not None:
                cols_to_show.append(original_row_headers[awb_col_idx])
            
            # Add up to 3 date headers to keep the console view readable
            count = 0
            for d_idx in date_col_indices:
                if count < 3: 
                    cols_to_show.append(original_row_headers[d_idx])
                    count += 1

            # Adjust Pandas settings so columns don't wrap awkwardly in your console window
            ...
            print("\n================────────────────────────────────────────================\n")
            # ──────────────────────────────────
            # df.to_csv('mis_cleaned.CSV', header=False, index=False)
            print("Success!")
