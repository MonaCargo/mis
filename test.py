# """
# flight_planning_extractor.py
# ==============================
# Extracts Export Planning Report PDF → pandas DataFrame only.
# No Excel output. Ready for DB pipeline.

# Usage (standalone):
#     python flight_planning_extractor.py Flight_planning_report.pdf

# Usage (as module):
#     from flight_planning_extractor import extract_flight_planning
#     df = extract_flight_planning("Flight_planning_report.pdf")

# DataFrame columns returned:
#     FLIGHT_NUM   | str   | Flight number (e.g. AI2380)
#     FLIGHT_DATE  | date  | Flight date (e.g. 2026-04-02)
#     DEST_SUMMARY | str   | Destination from summary block (e.g. SIN)
#     BOOKED_WGT   | float | Booked weight from summary block (e.g. 1006.0)
#     ULD_TYPE     | str   | ULD type from summary block (e.g. PMC)
#     TOTAL_ULD    | int   | Total number of ULDs
#     BULK_ALLOWED | bool  | Whether bulk is allowed (Y/N)
#     SLNO         | int   | Serial number of the AWB line
#     AWB_NUM      | str   | Air Waybill number (e.g. 09830567600)
#     LOCATION     | str   | Storage location code (e.g. EH_4_C)
#     LOC_PCS      | int   | Number of pieces at this location
#     GROSS_WGT    | float | Gross weight (kg)
#     CHG_WGT      | float | Chargeable weight (kg)
#     ORG          | str   | Origin airport code (e.g. DEL)
#     DES          | str   | Destination airport code (e.g. SIN)
#     SHC          | str   | Special handling codes (e.g. GEN, EAP,TRM,SP)
#     NOG          | str   | Nature of goods (e.g. MEDICINE (N))
#     PRIORITY     | str   | Priority flag (nullable)
...
# # Verified against Flight_planning_report.pdf character positions.
# COL_BOUNDS = [
#     ("SLNO",      4,   30),    # e.g. '1'@x=9, '10'@x=9-15
#     ("AWB_NUM",  34,  115),    # e.g. '098-30567600P'@x=36-108
#     ("LOCATION", 115, 180),    # e.g. 'EH_4_C'@x=121-157
#     ("LOC_PCS",  180, 230),    # e.g. '14'@x=189-201
#     ("GROSS_WGT",230, 310),    # e.g. '93.0'@x=249-273
#     ("CHG_WGT",  310, 360),    # e.g. '100.0'@x=316-346
#     ("ORG",      360, 400),    # e.g. 'DEL'@x=369-387
#     ("DES",      400, 432),    # e.g. 'SIN'@x=409-427
#     ("SHC",      435, 500),    # e.g. 'GEN'@x=436-454
#     ("NOG",      500, 660),    # e.g. 'MEDICINE (N)'@x=504-576
#     ("PRIORITY", 660, 800),
# ]

# # ULD summary block columns (DES / booked-wgt / ULD-type / total-ULD / bulk)
# _ULD_SUMMARY_COLS = [
#     ("DEST_SUMMARY",  24,  65),
#     ("BOOKED_WGT",    65, 200),
#     ("ULD_TYPE",     200, 375),
...
# # AWB format: 3digits-8digits with optional trailing P
# _AWB_RE = re.compile(r"^\d{3}-\d{7,8}P?$")

# # Flight-level header (spaces collapsed in extracted text)
# # e.g. "FLIGHTNUM:AI2380FLIGHTDATE:02APR2026"
...
# # Multi-line NOG continuation (e.g. "X PER" on its own line)
...
#     Returns
#     -------
#     pd.DataFrame ready for DB insert. Empty DataFrame if no records found.
#     """
#     import io
...
#     # Per-flight context — reset on each flight header
...
#             # Group chars into lines by y-position (quantised to 3pt buckets)
#             line_map: dict = {}
#             for ch in chars:
...
#                 lc = line_map[y_key]                    # line chars
#                 lt = _chars_to_text(lc)                  # line text (collapsed)
...
#                 # ── Data row ─────────────────────────────────────────────
#                 row = {col: _extract_cell(lc, x0, x1)
...
#     if len(sys.argv) < 2:
#         print("Usage: python flight_planning_extractor.py <path_to_pdf>")
#         sys.exit(1)

#     df = extract_flight_planning(sys.argv[1])
